            ...
            {products.length > 0 ? (
                <div className='container mx-auto h-auto grid grid-cols-5 gap-8'>
                    <div className='w-full h-full bg-white px-4 col-span-4'>
                        ...
                        <div>
                            {products.map((item) => (
                                <div key={item.id} className='w-full border-b-[1px] border-b-gray-300 p-4 flex-items-centre gap-6'>
                    ...
                    <div className='w-full h-52 bg-white col-span-1 flex flex-col justify-center items-center p-4'>
                        <div>
                            <p className='flex gap-2 items-start text-sm'>
                                <span>
                                    <CheckCircleIcon className='bg-white text-green-500 rouunded-full' />
                                </span>{" "}
                ...
                <div className='flex justify-center items-center gap-4 py-10'>
                    <div>
                        <img className="w-80 rounded-lg p-4 mx-auto" src={emptyCart} alt='CartImg'/>
                    </div>
                    <div className='w-96 p-4 bg-white flex flex-col items-center rounded-md shadow-lg'>
                        <h1 className='font-titleFont text-xl font-bold'>Your Cart Feels Lonely</h1>
                        <p className='text-sm text-center'>{" "}
                        Your Shopping cart lives to serv. Give it purpose - fill it with books, electronics, videos, etc. and make it happy.</p>
                    <Link to="/">
                    <button className="mt-6 bg-yellow-400 rounded-md cursor-pointer hover:bg-yellow-500 active:bg-yellow-700 px-8 py-2 fonnt-titleFont font-semibold text-lg"> Continue Shopping</button>
                    </Link>
                    </div>
                </div>)}
        </div>
